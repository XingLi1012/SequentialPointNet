...
class NTU_RGBD(Dataset):
    ...
    def __init__(self, root_path, opt,
        ...
        self.point_vids = os.listdir(self.root_path+'\\T120(ground)')#.sort()
        self.point_vids.sort()
        #print(self.point_vids)
        self.TRAIN_IDS = TRAIN_IDS
        if opt.dataset == 'ntu60':
            indx = self.point_vids.index('S017C003P020R002A060.npy')#('S016C003P040R002A060_xyzC.mat')#('S017C003P020R002A060.npy')
            self.point_vids = self.point_vids[0:indx]
            self.TRAIN_IDS = TRAIN_IDS_60
        elif opt.dataset == 'ntu120':
            indx = self.point_vids.index('S032C003P106R002A120.npy')#('S016C003P040R002A060_xyzC.mat')#('S017C003P020R002A060.npy')
            self.point_vids = self.point_vids[0:indx]
            self.TRAIN_IDS = TRAIN_IDS
    ...
    def __getitem__(self, idx):
        ...
        path_T=self.root_path+'\\T120(ground)'
        ...
        if(1==1):#
            ...
            for jj in range(self.framenum):
                iii =int(np.random.randint(int(self.all_framenum*jj/self.framenum), int(self.all_framenum*(jj+1)/self.framenum)))
                frame_index.append(iii)
        if(1==0):#
            frame_index = random.sample(list(all_sam),self.framenum)
        points4DV_T= np.load(path_cloud_npy_T)[frame_index,0:self.EACH_FRAME_SAMPLE_NUM,:self.INPUT_FEATURE_NUM]#60*512*4
    ...
    def point_transform(self,points_xyz,y):
        ...
        points_xyz[:,-(self.EACH_FRAME_SAMPLE_NUM-self.T_sample_num_level1):,:] = self.random_dropout_point_cloud(points_xyz[:,-(self.EACH_FRAME_SAMPLE_NUM-self.T_sample_num_level1):,:])
    ...
    def jitter_point_cloud(self, data, sigma=0.01, clip=0.05):
        ...
        jittered_data = np.clip(sigma * np.random.randn(M, N, C), -1 * clip, clip).astype(np.float32)#x
    ...
    def random_dropout_point_cloud(self, data):
        ...
        dropout_ratio = 0.7+ np.random.random()/2#np.random.random()
